# params
$template = "C:\Users\agent\OneDrive\Documents\Scripts\BrookSparkCoverLetter.docx"
$tempFolder = $env:TEMP + "\coverLetter-DOCX"
$Position_Name = Read-Host -Prompt 'Enter Position Name'
$Company_Name = Read-Host -Prompt 'Enter Company Name'
$Soft_Skills = Read-Host -Prompt 'Enter soft skills related to position, seperated by commas'
...
$body = $body.Replace("(Soft_Skills)", $Soft_Skills)
$body | Out-File $bodyFile -Force -Encoding ascii

# zip DOCX
$destfile = $template.Replace(".docx", "-current.docx")
